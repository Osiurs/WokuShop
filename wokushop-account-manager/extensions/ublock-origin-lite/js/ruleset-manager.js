...
export async function getDefaultRulesetsFromEnv() {
    ...
    for ( const lang of ((navigator.languages && Array.isArray(navigator.languages) ? navigator.languages : [navigator.language || 'en']).map(dropCountry)) ) {
        langSet.add(lang);
    }
    try {
        const uiLang = (typeof i18n?.getUILanguage === 'function') ? i18n.getUILanguage() : (navigator.language || 'en');
        langSet.add(dropCountry(uiLang));
    } catch (e) {
        langSet.add('en');
    }
