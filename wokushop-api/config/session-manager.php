 ...
 * Handles session lifecycle, IP restrictions, and concurrent session management
 *
 * @author Claude Code
 * @date 2025-01-04
 * @version 1.0
...
class SessionManager {
    ...
    public function canUserLogin($userId, $currentIP) {
        ...
        return [
            ...
            'instructions' => 'Use force logout API or wait for session timeout'
        ];
    }
    ...
    public function createSession($userId, $token, $ipAddress) {
        try {
            ...
            $this->db->rollBack();
            error_log("[SessionManager] Create session error: " . $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
    ...
    public function logoutSession($userId, $token) {
        try {
            ...
            // Log logout event
            $this->logSecurityEvent($userId, 'session_logout', [
            ...
            $this->db->rollBack();
            error_log("[SessionManager] Logout error: " . $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
    ...
    public function forceLogoutSession($sessionId) {
        ...
        if ($sessionInfo) {
            $this->logSecurityEvent($sessionInfo['user_id'], 'session_force_logout', [
                'session_id' => $sessionId,
    ...
    public function forceLogoutAllUserSessions($userId) {
        ...
        if ($affectedRows > 0) {
            $this->logSecurityEvent($userId, 'session_force_logout_all', [
    ...
    private function logSecurityEvent($userId, $action, $details = []) {
        try {
            ...
            error_log("[SessionManager] Log security event error: " . $e->getMessage());
        }
    }
